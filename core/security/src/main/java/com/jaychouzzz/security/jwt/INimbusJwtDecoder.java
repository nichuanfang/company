package com.jaychouzzz.security.jwt;/*
 ...
 * @since 5.2
 */
public final class INimbusJwtDecoder implements JwtDecoder {
     ...
     */
    public INimbusJwtDecoder(JWTProcessor<SecurityContext> jwtProcessor) {
        Assert.notNull(jwtProcessor, "jwtProcessor cannot be null");
        this.jwtProcessor = jwtProcessor;
    }

    public static JwkSetUriJwtDecoderBuilder withContext(ApplicationContext applicationContext) {
    ...
    public static final class JwkSetUriJwtDecoderBuilder {
        ...
        JWTProcessor<SecurityContext> processor() {
            SimpleClientHttpRequestFactory factory = (SimpleClientHttpRequestFactory) ((RestTemplate) restOperations).getRequestFactory();
            //set proxy
            if(StrUtil.isNotBlank(applicationContext.getEnvironment().getProperty("jaychouzzz.security.proxy.enable-proxy"))) {
                String host = applicationContext.getEnvironment().getProperty("jaychouzzz.security.proxy.host");
                String port = applicationContext.getEnvironment().getProperty("jaychouzzz.security.proxy.port");
                if(StrUtil.isBlank(host)) {
                    //不配置设置默认值
                    host = new ProxyProperties().getHost();
                }
                if(StrUtil.isBlank(port)) {
                    //不配置设置默认值
                    port = String.valueOf(new ProxyProperties().getPort());
                }
         ...
         */
        public INimbusJwtDecoder build() {
            return new INimbusJwtDecoder(processor());
        }
        ...
        public JwkSetUriJwtDecoderBuilder withContext(ApplicationContext applicationContext) {
